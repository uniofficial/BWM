...
public class ItemImageServiceImpl implements ItemImageService{
    ...
    public List<ItemImageResponse> uploadItemImages(Integer itemId, String sellerEmail, List<MultipartFile> images, Integer representativeIndex){
        ...
        Item item = itemRepository.findById(itemId).orElseThrow(() -> {throw new ItemNotFoundException("존재하지 않는 상품입니다. id = " + itemId);});
     ...
     *
     * 원본 파일명을 그대로 쓰지 않고 UUID로 바꾸는 이유: 같은 이름 파일이 덮어써지는 것 방지 + 경로 조작(path traversal) 방지
     */
